...
define(["util", "scene"], 
    ...
    var SceneController = function(context, scene) {
        ...
        this.canvas.addEventListener('onSelection',	(function(ev) { ev.preventDefault(); _controller.onSelection(ev);	}), false);
    };
    ...
    SceneController.prototype.onSelection = function(func) {
        ...
        if(!func) {
            throw new "SceneController.onSelection(): no function provided";
        };
     ...
     *  which is the currently selected object.
     */
    SceneController.prototype.onObjChange = function(func, params) {

		//console.log("func?", func);

        if(!func) {
            throw new "SceneController.onObjChange(): no function provided";
        };
        this.changeCallback = func;
		this.changeCallback && this.changeCallback(params);
    ...
    SceneController.prototype.select = function(obj) {
		...
		// performance: move draggers out of sight instead of delete on deselect?
        var draggers = obj.createDraggers();
		var tickMarks;
		if(obj.createTickMarks && obj.showTickMarks === true){
			tickMarks = obj.createTickMarks();
		}

        // store object and its draggers in an internal list
        this.selected.push( { "obj": obj, "draggers": draggers, "tickMarks": tickMarks } );

        // add draggers as scene objects so they get rendered
        this.scene.addObjects(draggers);

		// add tickmarks as scene objects so they get rendered
        if(tickMarks && tickMarks.length > 0 && obj.showTickMarks){
			this.scene.addObjects(tickMarks);
		}
    ...
    SceneController.prototype.deselect = function(obj) {
        ...
        for(var i=this.selected.length-1; i>=0; i-=1) {
            ...
            if(!obj || this.selected[i].obj == obj) {
				...
				// remove tickmarks from scene
				if (this.selected[i].tickMarks) {
					this.scene.removeObjects(this.selected[i].tickMarks);
				}
